...
int main(int argc, char *argv[]) {
  ...
  nn = 0; slen = 0; ipath = 0; ng = 0; num_seq = 0; max_phase = 0; 
  max_score = -100.0; piped = 0; max_slen = 0; gene_len_flag = 0;
  input_ptr = stdin; output_ptr = stdout;
  start_ptr = NULL; amino_ptr = NULL; nuc_ptr = NULL; summ_ptr = NULL;
  ...
  tinf.st_wt = 4.35;
  if(genetic_code == -1) tinf.trans_table = 11; /* 11 default genetic code */
  else tinf.trans_table = genetic_code;
  if(outfmt == -1) outfmt = 3; /* GFF default output format */

  /* Print header */
  if(quiet == 0) header(mode);
  ...
  if(train_file[0] != '\0') {
    ...
    if(read_training_file(train_file, &tinf) == -1) {
      perror("\n\nError: training file did not read correctly!"); exit(6); 
    }
    if(quiet == 0) {
      fprintf(stderr, "done.\n"); 
      fprintf(stderr, "-------------------------------------\n");
  ...
  ***************************************************************************/
  if(mode == 1 || (mode == 0 && train_file[0] == '\0')) {
    if(quiet == 0) fprintf(stderr, "Reading in the sequence(s) to train...");
    ...
    ***********************************************************************/
    ...
    if(mode == 1) {
      ...
      if(write_training_file(output_file, &tinf) != 0) {
        perror("\nError: could not write training file!"); exit(12); 
      }
      else { 
        if(quiet == 0) fprintf(stderr, "done.\n"); 
        exit(0); 
    ...
    if(fseek(input_ptr, 0, SEEK_SET) == -1) {
      perror("\nError: could not rewind input file."); exit(13);
    }
  ...
  while((slen = next_seq_multi(input_ptr, seq, useq, &num_seq, &gc, 
    ...
    if(mode != 2) { /* Single Genome Version */
      ...
      ***********************************************************************/
      ...
      tweak_final_starts(genes, ng, nodes, nn, tinf.st_wt);
      ...
      if(amino_ptr != NULL)
        write_translations(amino_ptr, genes, ng, nodes, seq, rseq, useq, slen,
                              tinf.trans_table, num_seq, short_header);
      if(nuc_ptr != NULL)
      ...
      for(i = 0; i < NUM_PRESET_GENOME; i++) { 
        ...
        record_overlapping_starts(nodes, nn, presets[i].tinf->st_wt, 1);
        ...
        if(nodes[ipath].score > max_score) {
          ...
          ng = add_genes(genes, nodes, ipath);
          tweak_final_starts(genes, ng, nodes, nn, presets[i].tinf->st_wt);
  ...
  if(amino_ptr != NULL) fclose(amino_ptr);
  if(nuc_ptr != NULL) fclose(nuc_ptr);
  if(summ_ptr != NULL) fclose(summ_ptr);
